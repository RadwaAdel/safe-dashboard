    ...
    <div class="container mt-2">
        <div class="row">
            <div class="col-lg-12 mt-3 mb-5 d-flex justify-content-between">
                <div class="pull-left">
                    <h4>جميع  ايصالات المصروفات</h4>
                </div>
                <div class="d-flex">
                    <div class="pull-right mb-2">
                        <a class="btn btn-outline-success" href="{{ route('expense.create') }}"> اضافة مصروف</a>
                    </div>
                    <div class="pull-right mb-2 mr-2">
                        <a class="btn btn-outline-primary" href="{{ route('exreceipt.create') }}"> اضافة ايصال مصروف</a>
                    </div>
                </div>
            </div>
        ...
        <table id="example" class="table table-bordered">
            ...
            @foreach ($transactions as $transaction)
                <tr>
                    ...
                    <td>{{ $transaction->expense->expense_name ??'لايوجد'}}</td>
                    <td>
                        ...
                        @else
                            غير معروف
                        @endif
                    </td>                    <td>{{ $transaction->value ??'لايوجد'}}</td>
                    <td>{{ $transaction->check_num ??'لايوجد'}}</td>
                    <td>{{ $transaction->due_date ??'لايوجد'}}</td>
                    <td>{{ $transaction->bank->bank ??'لايوجد'}}</td>

                    <td>
                        <form action="{{ route('exreceipt.destroy',$transaction->id ) }}" method="Post" class="d-flex justify-content-center">
                            <a class="btn btn-primary ml-2" href="{{ route('exreceipt.edit',$transaction->id ) }}">
