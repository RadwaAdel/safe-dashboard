...
@section('body')
    ...
    <div class="container mt-2">
        <div class="row">
            <div class="col-lg-12 margin-tb">
                <div class="pull-right mb-2">
                    <h2>تعديل ايصال مصروف</h2>
                </div>
                <div class="pull-right">
                    <a class="btn btn-primary" href='{{route('expenses.index')}}'> رجوع</a>
                </div>
            </div>
        </div>
        @if(session('status'))
        ...
        @endif
        <form action="{{ route('exreceipt.update', $transaction->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <select class="form-control m-bot15 main_select" name="expense_id" >
                        ...
                        @if ($expenses->count() > 0)
                            @foreach ($expenses as $expense)
                                <option value="{{$expense->id}}" {{$expense->id==$transaction->expense_id?'selected':''}}>
                                    {{ $transaction->expense ? $transaction->expense->expense_name : '' }}
                                </option>
                            @endforeach
                        @else
                            <option disabled selected>لا يوجد سجلات</option>
