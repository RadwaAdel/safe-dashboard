    ...
    <div class="container pt-2">
        <div class="row">
            <div class="col-lg-12 mt-3 mb-5 d-flex justify-content-between">
                ...
                <div class="pull-right mb-2 mr-2">
                    <a class="btn btn-outline-primary" href="{{ route('receipt.index') }}"> جميع ايصالات الايرادات</a>
                </div>
                </div>
            </div>
        </div>
        ...
        <table id="example" class="table table-bordered">
            ...
            <tr>
                ...
                <th  class="text-right" width="160px">اجراء</th>
            </tr>
            </thead>
            <tbody>
            @foreach ($revenues as $revenue)
                <tr>
                    <td>{{ $revenue->id }}</td>
                    <td>{{ $revenue->user->name ??'None'}}</td>
                    <td>{{ $revenue->revenue_name }}</td>
                    <td>
                        <form action="{{ route('revenues.destroy',$revenue->id) }}" method="Post">
                            <a class="btn btn-primary" href="{{ route('revenues.edit',$revenue->id) }}">
