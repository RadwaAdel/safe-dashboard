    ...
    <div class="container mt-2">
        <div class="row">
            <div class="col-lg-12 mt-3 mb-5 d-flex justify-content-between">
                <div class="pull-left">
                    <h4>جميع ايصالات الايرادات</h4>
                </div>
                <div class="d-flex">
                <div class="pull-right mb-2">
                    <a class="btn btn-outline-success" href="{{ route('revenues.create') }}"> اضافة ايراد</a>
                </div>
                <div class="pull-right mb-2 mr-2">
                    <a class="btn btn-outline-primary" href="{{ route('receipt.create') }}"> اضافة ايصال ايراد</a>
                </div>
                </div>
            </div>
        </div>
        ...
        <table id="example" class="table table-bordered">
            ...
            <tr>
                ...
                <th>قيمة الدفع</th>
                <th class="text-right">اسم الايراد</th>
                <th class="text-right">طريقة الدفع</th>
                <th class="text-right">قيمة الايراد </th>
                <th class="text-right">رقم الشيك </th>
                <th class="text-right">تاريخ الاستحقاق</th>
                <th class="text-right">اسم البنك</th>
                <th  class="text-right" width="280px">اجراء</th>
            </tr>
            </thead>
            <tbody>
            @foreach ($transactions as $transaction)
                <tr>
                    ...
                    <td>{{ $transaction->receipt_date ??'لايوجد'}}</td>
                    <td>{{ $transaction->company->company }}</td>
                    <td>{{$transaction->pay}}</td>
                    <td>{{ $transaction->revenues->revenue_name ??'لايوجد'}}</td>
                    <td>
                        @if ($transaction->revenue_type == 2)
                            شيك
                        @elseif ($transaction->revenue_type == 1)
                            كاش
                        @else
                            غير معروف
                        @endif
                    </td>
                    <td>{{ $transaction->value ??'لايوجد'}}</td>
                    <td>{{ $transaction->check_num ??'لايوجد'}}</td>
                    <td>{{ $transaction->due_date ??'لايوجد'}}</td>
                    <td>{{ $transaction->bank->bank ??'لايوجد'}}</td>

                    <td>
                        <form action="{{ route('receipt.destroy',$transaction->id ) }}" method="Post">
                            <a class="btn btn-primary ml-2" href="{{ route('receipt.edit',$transaction->id ) }}">
