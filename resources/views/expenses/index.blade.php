    ...
    <div class="container mt-2">
        <div class="row">
            <div class="col-lg-12 mt-3 mb-5 d-flex justify-content-between">
                ...
                <div class="pull-right mb-2 mr-2">
                    <a class="btn btn-outline-primary" href="{{ route('exreceipt.index') }}"> جميع ايصالات المصروفات</a>
                </div>
            </div>
            </div>
        </div>
        ...
        <table id="example" class="table table-bordered">
            ...
            @foreach ($expenses as $expense)
                <tr>
                    <td>{{ $expense->id }}</td>
                    <td>{{ $expense->user->name ??'None'}}</td>
                    <td>{{ $expense->expense_name }}</td>
                    <td>
                        <form action="{{ route('expense.destroy',$expense->id) }}" method="Post">
                            <a class="btn btn-primary" href="{{ route('expense.edit',$expense->id) }}">
